...
Создан пользователь `zahar`:

```bash
sudo adduser zahar
sudo usermod -aG sudo zahar
groups zahar  # zahar : zahar sudo users docker
```

Настройка SSH-доступа для `zahar`:

```bash
sudo mkdir -p /home/zahar/.ssh
sudo cp /home/cxuser/.ssh/authorized_keys /home/zahar/.ssh/
sudo chown -R zahar:zahar /home/zahar/.ssh
sudo chmod 700 /home/zahar/.ssh
sudo chmod 600 /home/zahar/.ssh/authorized_keys
```

**Результат:** Пользователь `zahar` имеет права sudo и может подключаться по SSH.
...
git config --global user.name "Zahar"
git config --global user.email "zahar@example.com"
```
...
3. Открытие папки: `/home/zahar/django_project`
...
<img width="1280" height="973" alt="заметка 1" src="https://github.com/user-attachments/assets/089d1aa6-52cc-401b-ad86-2e0d16822b26" />
